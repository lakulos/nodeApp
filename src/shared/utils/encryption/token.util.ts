...
export class TokenGeneratorUtil {
  ...
  generateReference(
    refType: string,
    length = this.configService.get<number>(
      'common.auth.defaultRandomStringLength',
    ),
  ): string {
    /**
    ...
    return `${refType}${cryptoRandomString({
      length,
      characters: this.configService.get<string>(
        'common.auth.defaultRandomStringCharacters',
      ),
    })}`;
  }
